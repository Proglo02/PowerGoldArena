...
{
    public class FictionalMapCameraManager : MonoBehaviour
    {
        ...
        private void Update()
        {
            for (int i = 0; i < NumCameras; i++)
            {
                if (Vector3.Distance(_fictionalCameras[i].transform.position, _fictionalCameras[i].targetPosition) < radius)
                {
                    GetNewTarget(i, 0);
                }
